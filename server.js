  ...
  ws.on('message', (raw) => {
    ...
    switch (msg.type) {
      ...
      case 'ready': {
        ...
        if (room.gameStarted) break; // ya en curso, ignorar

        const readyPlayers = Object.values(room.players).filter(p => p.ready);

        if (readyPlayers.length >= 2) {
          // Cancelar timeout solo si había uno pendiente y arrancar
          if (room._soloTimeout) { clearTimeout(room._soloTimeout); room._soloTimeout = null; }
          launchGame(room);
        } else if (readyPlayers.length === 1) {
          // Solo un jugador listo: esperar 30s y arrancar en solitario si nadie más llega
          if (!room._soloTimeout) {
            room._soloTimeout = setTimeout(() => {
              if (!room.gameStarted) {
                const stillReady = Object.values(room.players).filter(p => p.ready);
                if (stillReady.length >= 1) {
                  console.log(`Sala ${room.id}: timeout, arrancando con ${stillReady.length} jugador(es)`);
                  launchGame(room);
                }
              }
            }, 30000);
