...
const httpServer = http.createServer((req, res) => {
  ...
  // Serve GLB skin models from /models/ folder
  if (req.url.startsWith('/models/') && req.url.endsWith('.glb')) {
    const fileName = path.basename(req.url);                    // e.g. ironman.glb
    const filePath = path.join(__dirname, 'models', fileName);  // ./models/ironman.glb
    fs.readFile(filePath, (err, data) => {
      if (err) { res.writeHead(404); res.end('Model not found'); return; }
      res.writeHead(200, {
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    switch (msg.type) {
      ...
      case 'ready': {
        ...
        if (readyPlayers.length >= 2) launchGame(room);
        break;
      }
  ...
  ws.on('close', () => {
    ...
    if (room.gameStarted) {
      const alive = Object.values(room.players).filter(p => p.alive);
      if (alive.length === 1) room.sendTo(alive[0].id, { type: 'you_won' });
    }
  });
